...
#os.environ['OptiX_INSTALL_DIR'] = '/optix77'
import torch
import argparse
...
if __name__ == '__main__':
    ...
    ### training options
    parser.add_argument('--iters', type=int, default=7500, help="training iters")
    ...
    parser.add_argument('--n_ckpt', type=int, default=5, help="save $ times during training")
    ...
    parser.add_argument('--lambda_normal', type=float, default=0.0, help="loss scale")
    parser.add_argument('--lambda_edgelen', type=float, default=0.0, help="loss scale")

    # brdf part
    parser.add_argument('--use_brdf', action='store_true', help="use brdf rendering to get material")
    parser.add_argument('--use_restir', action='store_true', help="use restir render")
    parser.add_argument('--use_bi_de', action='store_true', help="use bilateral denoiser")
    parser.add_argument('--use_hdr', action='store_true', help="use reinhard tone mapping to handle hdr")
    parser.add_argument('--exposure', type=float, default=0.0, help="only will be used with hdr")
    parser.add_argument('--learning_rate_mat', type=float, default=0.03, help="loss scale")
    parser.add_argument('--learning_rate_lgt', type=float, default=0.09, help="loss scale")
    parser.add_argument('--lambda_rgb_brdf', type=float, default=0.02, help="loss scale")
    parser.add_argument('--lambda_brdf_diffuse', type=float, default=0.0015, help="loss scale")
    parser.add_argument('--lambda_brdf_specular', type=float, default=0.000025, help="loss scale")
    parser.add_argument('--lambda_kd', type=float, default=0.005, help="loss scale")
    parser.add_argument('--lambda_ks', type=float, default=0.0025, help="loss scale")
    parser.add_argument('--lambda_nrm', type=float, default=0.00025, help="loss scale")
    parser.add_argument('--lambda_chroma', type=float, default=0.0, help="loss scale")
    parser.add_argument('--lambda_net_specular', type=float, default=0.09, help="loss scale")# not used right now
    parser.add_argument('--spp', type=int, default=32, help="spp")
    parser.add_argument('--roughness_min', type=float, default=0.08, help="roughness min")
    parser.add_argument('--me_max', type=float, default=0.0, help="metallic max")
    parser.add_argument('--env_h', type=int, default=256, help="envmap height")
    parser.add_argument('--env_w', type=int, default=512, help="envmap width")
    parser.add_argument('--lambda_extra_kd', type=float, default=0.0, help="loss scale")
    parser.add_argument('--eval_use_gt_mask', action='store_true', help="use gt mask to eval")
    #for relighting
    parser.add_argument('--envmap_path', type=str, default='None', help="envmap path for relighting")
    parser.add_argument('--albedo_scale_x', type=float, default=1.0, help="scale albedo")
    parser.add_argument('--albedo_scale_y', type=float, default=1.0, help="scale albedo")
    parser.add_argument('--albedo_scale_z', type=float, default=1.0, help="scale albedo")
    ...
    #opt.use_restir = True
    #opt.use_bi_de = False
    #opt.eval_use_gt_mask = True

    opt.kd_min = [ 0.0,  0.0,  0.0,  0.0]
    opt.kd_max = [ 1.0,  1.0,  1.0,  1.0]
    opt.ks_min = [ 0.0,  opt.roughness_min, 0.0]
    opt.ks_max = [ 0.0,  1.0,  opt.me_max]
    opt.light_probe_res_hw = [opt.env_h, opt.env_w]
    ...
    if opt.test:
        ...
        trainer = Trainer('ngp', opt, model, frame_dim=test_loader.H_W, device=device, workspace=opt.workspace, criterion=criterion, fp16=opt.fp16, use_checkpoint=opt.ckpt)

        if opt.gui:
            #gui = NeRFGUI(opt, trainer)
            #gui.render()
            print(' no implement for GUI now, sorry')
        ...
        if opt.gui:
            ...
            trainer.metrics_brdf = [PSNRMeter_brdf(),]
            trainer.train(train_loader, valid_loader, max_epoch)
            
            # last validation
            trainer.metrics = [PSNRMeter(), SSIMMeter(), LPIPSMeter(device=device)]
            trainer.metrics_brdf = [PSNRMeter_brdf(), SSIMMeter_brdf(), LPIPSMeter_brdf(device=device)]
